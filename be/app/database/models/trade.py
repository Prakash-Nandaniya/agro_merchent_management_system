...
class Trade(Base):
    ...
    trade_creation_date: Mapped[datetime] = mapped_column(DateTime(timezone=True), server_default=func.now(), nullable=False)
    ...
    gst_collected: Mapped[Decimal] = mapped_column(Numeric(12, 2),nullable=False, default=Decimal("0.00"))
    tds_deducted: Mapped[Decimal] = mapped_column(Numeric(12, 2),nullable=False, default=Decimal("0.00"))
    mill_payment: Mapped[Decimal] = mapped_column(Numeric(12, 2),nullable=False, default=Decimal("0.00"))

    # ── Outflow ────────────────────────────────────────────────────────────
    farmer_payment: Mapped[Decimal] = mapped_column(Numeric(12, 2),nullable=False, default=Decimal("0.00"))
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "mill_qty > 0", name="ck_trades_mill_qty_positive"
        ),
        CheckConstraint(
            "mill_rate > 0", name="ck_trades_mill_rate_positive"
        ),
        CheckConstraint(
        ...
        CheckConstraint(
            "labour_cost >= 0", name="ck_trades_labour_cost_non_negative"
        ),
        CheckConstraint(
            "other_cost >= 0", name="ck_trades_other_cost_non_negative"
        ),
    )
